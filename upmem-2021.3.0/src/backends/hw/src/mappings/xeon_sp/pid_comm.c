...
#define GATHER_COPY(iter, src_rank_bgwise_addr, dst_rank_bgwise_addr)                           \
    ...
    {                                                                                           \
        ...
        __m512i mask = _mm512_set_epi64(                                                        \
                                        ...
                                        0x0f0b07030e0a0602ULL,                                  \
                                        0x0d0905010c080400ULL);                                 \
        __m512i vindex = _mm512_setr_epi32(0, 8, 16, 24, 32, 40, 48, 56,                        \
        ...
        for(uint32_t i=0; i<1; i++){                                                            \
            reg1 = _mm512_loadu_si512((void *)(src_rank_clwise_addr));                          \
                                                                                                \
            reg1 = _mm512_i32gather_epi32(vindex, (int64_t*)&reg1, 1);                          \
            reg1 = _mm512_shuffle_epi8(reg1, mask);                                             \
            ...
            {                                                                                   \
                ...
                for(uint32_t i=0; i<num_iter_src; i++){                                         \
                    ...
                    sum = _mm512_add_epi8(sum, reg1);                                           \
                                                                                                ...
                                                                                                \
                sum = _mm512_i32gather_epi32(vindex, (int64_t*)&sum, 1);                        \
...
void xeon_sp_trans_reduce_rg(void **base_region_addr_src, void *base_region_addr_dst, uint32_t* src_rg_id, uint32_t dst_rg_id, uint32_t src_start_offset, uint32_t dst_start_offset, uint32_t byte_length, uint32_t num_iter_src, uint32_t alltoall_comm_type, uint32_t communication_buffer_offset, void **host_buffer, uint32_t host_buffer_first_index, uint32_t data_type){
    base_region_addr_dst+=0;
    dst_rg_id+=0;
    communication_buffer_offset+=0;
    void *src_rank_base_addr;
    ...
    uint32_t iter_length= byte_length/64; 
    ...
    if(alltoall_comm_type==0){ //parallel to the entangled group
        for(uint32_t i=0; i<iter_length; i++){
            ...
            for(uint32_t j=0; j<num_iter_src; j++){
                ...
                for(uint32_t k=0; k<8; k++){
                    mram_64_bit_word_offset = apply_address_translation_on_mram_offset_a2a(src_mram_offset + byte_length/8*k);
                    next_data = BANK_OFFSET_NEXT_DATA_a2a(mram_64_bit_word_offset);
                    mram_src_offset_1mb_wise = (next_data % BANK_CHUNK_SIZE_a2a) + (next_data / BANK_CHUNK_SIZE_a2a) * BANK_NEXT_CHUNK_OFFSET_a2a;
                    src_rank_addr = src_rank_base_addr + mram_src_offset_1mb_wise;
                    src_rank_addr_iter[j*8 + k] = src_rank_addr + src_rotate_group_offset_256_64;
        ...
        for(uint32_t i=0; i<iter_length; i++){
            ...
            for(uint32_t j=0; j<num_iter_src; j++){
                ...
                for(uint32_t k=0; k<1; k++){
                    mram_64_bit_word_offset = apply_address_translation_on_mram_offset_a2a(src_mram_offset + byte_length/8*k*0);
                    next_data = BANK_OFFSET_NEXT_DATA_a2a(mram_64_bit_word_offset);
                    mram_src_offset_1mb_wise = (next_data % BANK_CHUNK_SIZE_a2a) + (next_data / BANK_CHUNK_SIZE_a2a) * BANK_NEXT_CHUNK_OFFSET_a2a;
                    src_rank_addr = src_rank_base_addr + mram_src_offset_1mb_wise;
                    src_rank_addr_iter[j + k] = src_rank_addr + src_rotate_group_offset_256_64;
...
typedef struct {
    ...
    void *src_rank_base_addr;
    void **dst_rank_base_addr_arr;
    uint32_t communication_buffer_offset;
    uint32_t byte_length;
    uint32_t iter_length;
    uint32_t src_rotate_group_offset_256_64;
    uint32_t* dst_rotate_group_offset_256_64;
    uint32_t num_iter_dst;
    uint32_t num_thread;
    uint32_t thread_id;
}thread_ag_parameter_x;

void *thread_all_gather_x(void *thread_parameter){
    thread_ag_parameter_x *thread_ag_x_parameter = (thread_ag_parameter_x *)thread_parameter;
    uint32_t src_mram_offset = thread_ag_x_parameter -> src_mram_offset;
    uint32_t dst_mram_offset = thread_ag_x_parameter -> dst_mram_offset;
    uint32_t src_start_offset = thread_ag_x_parameter -> src_start_offset;
    uint32_t dst_start_offset = thread_ag_x_parameter -> dst_start_offset;
    void *src_rank_base_addr = thread_ag_x_parameter -> src_rank_base_addr;
    void **dst_rank_base_addr_arr = thread_ag_x_parameter -> dst_rank_base_addr_arr;
    uint32_t communication_buffer_offset = thread_ag_x_parameter -> communication_buffer_offset;
    uint32_t byte_length = thread_ag_x_parameter -> byte_length;
    uint32_t iter_length = thread_ag_x_parameter -> iter_length;
    uint32_t src_rotate_group_offset_256_64 = thread_ag_x_parameter -> src_rotate_group_offset_256_64;
    uint32_t* dst_rotate_group_offset_256_64 = thread_ag_x_parameter -> dst_rotate_group_offset_256_64;
    uint32_t num_iter_dst = thread_ag_x_parameter -> num_iter_dst;
    uint32_t num_thread = thread_ag_x_parameter -> num_thread;
    uint32_t thread_id = thread_ag_x_parameter -> thread_id;
...
void *thread_all_gather_xt(void *thread_parameter){
    thread_ag_parameter_x *thread_ag_x_parameter = (thread_ag_parameter_x *)thread_parameter;
    int offset = thread_ag_x_parameter -> src_mram_offset;
    if(offset) return 0;
    return 0;
}
