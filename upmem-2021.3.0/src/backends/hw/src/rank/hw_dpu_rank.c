...
void *thread_reduce_scatter_cpu_x_rns(void *thread_parameter){
    ...
    for(uint32_t k=start_point; k<(start_point+(share+remain_iter)); k = k + 1){
        ...
        printf("src rank id : %d, src_rg_id : %d            dst_rank_id : %d, dst_rg_id : %d\n", iter_b*a/64 + iter_c*a*b/64, src_rg_id[0], dst_rank_id, dst_rg_id);
...
void *thread_reduce_scatter_rns(void *thread_parameter){
    ...
    for(uint32_t i=start_point; i<(start_point+(share+remain_iter)); i++){
        ...
        printf("(src rank id, src_rg_id) :");
        for (uint32_t j=0; j<total_axis_product; j++) printf("(%d, %d) ", src_rank_id[j], src_rg_id[j]);
        printf("dst_rank_id : %d, dst_rg_id : %d\n", dst_rank_id, dst_rg_id);
...
//reduce
void *thread_reduce_x_rns(void *thread_parameter){
    st_thread_all_to_all_x_parameter *each_thread_all_to_all_x_parameter = (st_thread_all_to_all_x_parameter *)thread_parameter;
    uint32_t thread_id = each_thread_all_to_all_x_parameter->p_thread_id;
    struct dpu_set_t *comm_dpu_set=each_thread_all_to_all_x_parameter->p_comm_dpu_set;
    uint32_t src_start_offset=each_thread_all_to_all_x_parameter->p_src_start_offset;
    ...
    void **host_buffer=each_thread_all_to_all_x_parameter->p_host_buffer;
    uint32_t data_type=each_thread_all_to_all_x_parameter->p_target_dpu_index;
    uint32_t num_thread=each_thread_all_to_all_x_parameter->p_num_thread;
    ...
    for(uint32_t k=start_point; k<(start_point+(share+remain_iter)); k++){
        ...
        if(iter_dst_a==0){
            params_src->translate.trans_reduce_rg(rank_base_address_src, rank_base_address_dst, src_rg_id, dst_rg_id, src_start_offset_iter, dst_start_offset_iter, dpu_byte_length, num_iter_src, alltoall_comm_type, communication_buffer_offset, host_buffer, (k/(a/8)), data_type);
        }
    }
    return 0;
}
...
static dpu_rank_status_e
hw_reduce_x_rns(struct dpu_set_t *comm_dpu_set, uint32_t src_start_offset, uint32_t dst_start_offset, uint32_t dpu_byte_length, uint32_t a, uint32_t b, uint32_t c, uint32_t alltoall_comm_type, uint32_t communication_buffer_offset, uint32_t data_type, void ** host_buffer){
    ...
    for(uint32_t iter_thread=0; iter_thread<thread_num; iter_thread++){
        ...
        thread_params[iter_thread].p_alltoall_comm_type=alltoall_comm_type;
        thread_params[iter_thread].p_communication_buffer_offset=communication_buffer_offset;
        thread_params[iter_thread].p_target_dpu_index=data_type;
        thread_params[iter_thread].p_host_buffer=host_buffer;
        thread_params[iter_thread].p_num_thread=thread_num;
        pthread_create(&array_thread[iter_thread], NULL, thread_reduce_x_rns, (void *) &thread_params[iter_thread]);
    }
    for(uint32_t iter_thread=0; iter_thread<thread_num; iter_thread++){
        pthread_join(array_thread[iter_thread], (void **)&status);
    }
...
void *thread_reduce_y_rns(void *thread_parameter){
    st_thread_all_to_all_x_parameter *each_thread_all_to_all_x_parameter = (st_thread_all_to_all_x_parameter *)thread_parameter;
    uint32_t thread_id = each_thread_all_to_all_x_parameter->p_thread_id;
    struct dpu_set_t *comm_dpu_set=each_thread_all_to_all_x_parameter->p_comm_dpu_set;
    uint32_t src_start_offset=each_thread_all_to_all_x_parameter->p_src_start_offset;
    ...
    for(uint32_t k=start_point; k<(start_point+(share+remain_iter)); k++){
        ...
        if(iter_dst_b==0){
            params_src->translate.trans_reduce_rg(rank_base_address_src, rank_base_address_dst, src_rg_id, dst_rg_id, src_start_offset_iter, dst_start_offset_iter, dpu_byte_length, num_iter_src, alltoall_comm_type, communication_buffer_offset, host_buffer, k/b, data_type);
        }
...
static dpu_rank_status_e
hw_reduce_y_rns(struct dpu_set_t *comm_dpu_set, uint32_t src_start_offset, uint32_t dst_start_offset, uint32_t dpu_byte_length, uint32_t a, uint32_t b, uint32_t c, uint32_t alltoall_comm_type, uint32_t communication_buffer_offset, uint32_t data_type, void **host_buffer){
    ...
    for(uint32_t iter_thread=0; iter_thread<thread_num; iter_thread++){
        ...
        thread_params[iter_thread].p_host_buffer=host_buffer;
        thread_params[iter_thread].p_num_thread=thread_num;
        pthread_create(&array_thread[iter_thread], NULL, thread_reduce_y_rns, (void *) &thread_params[iter_thread]);
    }
    for(uint32_t iter_thread=0; iter_thread<thread_num; iter_thread++){
        pthread_join(array_thread[iter_thread], (void **)&status);
    }
...
void *thread_all_gather_rns(void *thread_parameter){
    ...
    for(uint32_t i=start_point; i<(start_point+(share+remain_iter)); i++){
        ...
        //printf("(src rank id, src_rg_id) : (%d, %d), (%d, %d), (%d, %d), (%d, %d)            dst_rank_id : %d, dst_rg_id : %d\n", src_rank_id[0], src_rg_id[0], src_rank_id[1], src_rg_id[1], src_rank_id[5], src_rg_id[5], src_rank_id[8], src_rg_id[8], dst_rank_id, dst_rg_id);
