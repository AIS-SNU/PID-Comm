...
// reconstruct weight for distribution to DPUs
void reconstruct_weight(struct Matrix *mat, struct dpu_info_t *dpu_info, uint32_t max_rows_per_dpu, uint32_t nr_of_partitions){
    uint32_t nrows = mat->nrows;
    uint32_t ncols = mat->ncols;
    uint32_t rows_per_dpu, prev_rows_dpu;
    int index;
        ...
        if(rows_per_dpu == max_rows_per_dpu){
            for(int j=0; j<ncols; j++){
                for(int k=0; k<max_rows_per_dpu; k++){
                    ...
                    temp[(i * ncols * max_rows_per_dpu) + (j * max_rows_per_dpu) + k] = mat->val[(ncols * prev_rows_dpu) + (k * ncols) + j];
                }
            }
        }
...
// reconstruct mid-matrix for distribution to DPUs
void reconstruct_mid(struct Matrix *mat, struct dpu_info_t *dpu_info, uint32_t max_rows_per_dpu, uint32_t nr_of_partitions){
    ...
    T* temp = malloc(ncols * max_rows_per_dpu * nr_of_partitions * sizeof(T));
    for(int i=0;i<nr_of_partitions; i++){
        rows_per_dpu = dpu_info[i*32].rows_per_dpu;
        prev_rows_dpu = dpu_info[i*32].prev_rows_dpu;
...
//read the COO matrix, and add padding to distribute to each DPU
