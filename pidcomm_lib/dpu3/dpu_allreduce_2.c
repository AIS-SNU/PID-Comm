...
BARRIER_INIT(my_barrier_4, NR_TASKLETS/4);
BARRIER_INIT(my_barrier_2, NR_TASKLETS/8);
    ...
    if(num_comm_dpu%8 == 0){
        ...
        for(int target_dpu_num = tasklet_id; target_dpu_num < num_comm_dpu; target_dpu_num = target_dpu_num + 8){
            for(int iteration =0; iteration < iter; iteration++){
                ...
                original_addr = (uint32_t) DPU_MRAM_HEAP_POINTER + start_offset + /* words[target_dpu_num] */ max_words_per_dpu * target_dpu_num *sizeof(T) + 2048*iteration;
                ...
                if(no_rotate){
                    target_addr = (uint32_t) DPU_MRAM_HEAP_POINTER + target_offset + /* words[target_dpu_num] */max_words_per_dpu * target_dpu_num*sizeof(T) + 2048*iteration;
                }
                else{
